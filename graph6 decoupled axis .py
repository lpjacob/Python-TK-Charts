...
class Graph(Canvas):
	...
	def drawAxisLabel(self):
		#add axis lables
		y=self.yOrigin
		yStart = 0 #value for axis labels to start at origin
		yinterval = 50 #pixel spacing between lables

		ylableInterveral = 0.25 #rate of change of lables
		while y > 0 :
			#strY = str(self.yOrigin-y)
			strY = str(yStart)
			self.create_text(self.xOrigin,y,text=strY,fill=self.axis_color)
			y-= yinterval
			yStart += ylableInterveral
